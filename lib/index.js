...
Node.createElement = function createElement(name) {
	return function createNode(one, two, three) {
		...
		if (typeOfOne === 'primitive' || typeOfOne === 'object') {
			attributes = one;
...
/* Provides a quasi-element for creating pure text content not in a tag.
 *
 * This method name is subject to change if the HTML standard ever comes out
 * with a `text` element. */
Node.text = function(content) {
	var node = Node.createNode('', null, [], {}, content, []);
...
Node.prototype.toDOM = function toDOM(document_) {
	...
	if (node.isTextNode) {
		return document_.createTextNode(node.content);
	}
	...
	if (isObject(node.events)) {
		Object.keys(node.events).forEach(function(action) {
			root.addEventListener(action, node.events[action]);
		});
	}
...
/* Adds an Event Handler to the given node. If the node is turned into a DOM
 * node, the event handler gets added as well. (Otherwise, the handler is
 * essentially ignored). */
Node.prototype.on = function on(action, fn) {
	var node = this;

	if (!node.events) {
		node.events = {};
	}
	node.events[action] = fn;
